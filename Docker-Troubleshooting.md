...
2. **开启全局VPN**：在系统层面开启全局代理模式
3. **混合测试**：不同镜像源与VPN开关的组合测试

**结果**：以上方案均未解决问题，错误依旧。

## 根本原因分析

Docker 守护进程运行在独立的网络命名空间中，不自动继承宿主机的系统代理设置。即使系统开启了全局VPN，Docker 容器内的网络请求仍会直连外网，导致无法访问镜像仓库。
...
- 此模式在系统底层创建虚拟网卡，强制所有网络流量（包括Docker）经过代理
- **效果**：一劳永逸，无需额外配置
